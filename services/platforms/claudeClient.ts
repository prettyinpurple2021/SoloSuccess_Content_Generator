...
export class ClaudeClient {
  ...
  async generateSocialMediaPost(topic: string, platform: string, tone: string, length: number): Promise<{ success: boolean; content?: string; error?: string }> {
  ...
  async generateContentIdeas(topic: string, count: number = 5): Promise<{ success: boolean; ideas?: string[]; error?: string }> {
  ...
  async optimizeContent(content: string, platform: string, targetAudience: string): Promise<{ success: boolean; optimizedContent?: string; suggestions?: string[]; error?: string }> {
...
- Call-to-action effectiveness`;
    ...
    if (result.success && result.content) {
      ...
      const optimizedContent = lines.find(line => line.includes('Optimized:') || line.includes('Version:'))?.replace(/.*?:/, '').trim() || content;
  ...
  async analyzeSentiment(content: string): Promise<{ success: boolean; sentiment?: string; confidence?: number; error?: string }> {
...
Content: "${content}"`;
  ...
  async generateHashtags(content: string, platform: string, count: number = 5): Promise<{ success: boolean; hashtags?: string[]; error?: string }> {
...
Return only the hashtags, one per line.`;
  ...
  async generateLongFormContent(topic: string, type: 'blog' | 'article' | 'guide', length: 'short' | 'medium' | 'long'): Promise<{ success: boolean; content?: string; error?: string }> {
...
- Include a compelling introduction and conclusion`;
  ...
  async summarizeContent(content: string, maxLength: number = 200): Promise<{ success: boolean; summary?: string; error?: string }> {
