...
export class GoogleAnalyticsClient {
  ...
  async testConnection(): Promise<{ success: boolean; error?: string; details?: any }> {
    try {
      ...
      const response = await fetch(`${this.baseUrl}/properties/${this.credentials.propertyId}/metadata`, {
        method: 'GET',
        headers: {
  ...
  async getPageViews(startDate: string, endDate: string): Promise<{
    totalViews: number;
    uniqueViews: number;
    ...
    try {
      const response = await fetch(`${this.baseUrl}/properties/${this.credentials.propertyId}:runReport`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${this.credentials.accessToken}`,
          'Content-Type': 'application/json'
      ...
      if (response.ok) {
        ...
        const totalViews = rows.reduce((sum: number, row: any) => 
          sum + parseInt(row.metricValues?.[0]?.value || '0'), 0);
        const uniqueViews = rows.reduce((sum: number, row: any) => 
          sum + parseInt(row.metricValues?.[1]?.value || '0'), 0);
        const avgTime = rows.reduce((sum: number, row: any) => 
          sum + parseFloat(row.metricValues?.[2]?.value || '0'), 0) / rows.length;
        const bounceRate = rows.reduce((sum: number, row: any) => 
          sum + parseFloat(row.metricValues?.[3]?.value || '0'), 0) / rows.length;
  ...
  async getTrafficSources(startDate: string, endDate: string): Promise<{
    organic: number;
    direct: number;
    social: number;
    referral: number;
  ...
  async getAudienceDemographics(startDate: string, endDate: string): Promise<{
  ...
  async getContentPerformance(startDate: string, endDate: string): Promise<{
