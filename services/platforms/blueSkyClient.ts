...
export class BlueSkyClient {
  ...
  async postText(text: string, replyTo?: string): Promise<{ success: boolean; postId?: string; url?: string; error?: string }> {
    try {
  ...
  async getTrendingTopics(categories?: string[]): Promise<any[]> {
    try {
      ...
      const response = await fetch(`${this.baseUrl}/xrpc/app.bsky.unspecced.getPopularFeedGenerators`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        }
  ...
  async getHashtagMetrics(hashtag: string): Promise<{
    ...
    try {
      ...
      console.error('Error fetching BlueSky hashtag metrics:', error);
      return {
        usageCount: 0,
        avgEngagement: 0,
        trendingScore: 0
  ...
  async getOptimizationData(): Promise<any> {
    try {
      ...
      console.error('Error fetching BlueSky optimization data:', error);
      return {
        platform: 'bluesky',
        bestPostingTimes: ['9:00', '12:00', '15:00', '18:00'],
        optimalContentLength: 300,
