...
const gapiReady = new Promise<void>((resolve) => {
    if (!isBloggerConfigured) {
        console.warn("Blogger integration is not configured. Missing GOOGLE_CLIENT_ID or GOOGLE_API_KEY environment variables.");
 ...
 */
export const createPost = async (blogId: string, title: string, content: string, labels: string[]): Promise<any> => {
    if (!isBloggerConfigured || !window.gapi?.client?.blogger) {
