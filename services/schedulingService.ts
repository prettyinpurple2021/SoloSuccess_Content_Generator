...
export class SchedulingService {
  ...
  async analyzeContentConflicts(posts: Post[]): Promise<ConflictAnalysis> {
    try {
      ...
      if (conflicts.length === 0) {
        suggestions.push('No scheduling conflicts detected. Your content schedule looks well-spaced.');
      } else {
        suggestions.push(`Found ${conflicts.length} potential conflicts. Review the suggestions below to optimize your schedule.`);
        
        if (conflicts.some(c => c.conflictType === 'timing')) {
          suggestions.push('Consider using bulk scheduling with automatic spacing to prevent timing conflicts.');
        }
        
        if (conflicts.some(c => c.conflictType === 'topic')) {
          suggestions.push('Diversify your content topics or create themed content series with proper spacing.');
        }
      }
  ...
  async updateTimingRecommendations(platform?: string): Promise<TimeSlot[]> {
    try {
      ...
      const updatedTimeSlots = await this.calculateOptimalTimesFromAnalytics(recentAnalytics, platform);
      ...
      const hasSignificantChange = this.detectSignificantTimingChanges(previousTimeSlots, updatedTimeSlots);
  ...
  async bulkSchedulePosts(
    ...
    try {
      ...
      const { startDate, endDate, platforms, spacing, customSpacingHours, respectOptimalTimes, avoidWeekends, targetTimezones } = options;
  ...
  private haveSameAudience(post1: Post, post2: Post): boolean {
    ...
    return post1.audienceProfileId === post2.audienceProfileId && 
           post1.audienceProfileId !== undefined;
  }
  ...
  private detectSignificantTimingChanges(
    ...
    const commonSlots = topPrevious.filter(prevSlot =>
      topNew.some(newSlot => 
        newSlot.dayOfWeek === prevSlot.dayOfWeek && 
        newSlot.time === prevSlot.time
      )
    );
  ...
  private findNextOptimalSlot(
    ...
    for (let i = startIndex; i < availableSlots.length; i++) {
      ...
      const matchingOptimalTime = optimalTimes.find(optimal => 
        optimal.dayOfWeek === slotDay && 
        parseInt(optimal.time.split(':')[0]) === slotHour
      );
