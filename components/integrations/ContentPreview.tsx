...
export const ContentPreview: React.FC<ContentPreviewProps> = ({
  ...
  useEffect(() => {
    if (content.trim() && selectedPlatforms.length > 0) {
      ...
      contentAdaptationService.adaptContentForMultiplePlatforms(content, selectedPlatforms, {
              ...
              activeTab === platform && (
                ...
                >
                  <div className="space-y-4">
                    {/* Platform Info */}
                    <div className={`bg-gradient-to-r ${getPlatformColor(platform)} rounded-lg p-4 text-white`}>
                        ...
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <p className="text-sm opacity-90">
                              {contentAdaptationService.getPlatformLimits(platform)?.maxCharacters || 0} character limit
                            </p>
                          </div>
                        </div>
                        {validations[platform] && (
                          <div className="text-right">
                            <div className={`text-lg ${getStatusColor(validations[platform].isValid)}`}>
                              {getStatusIcon(validations[platform].isValid)}
                            </div>
                            <div className="text-sm opacity-90">
                              {validations[platform].characterCount}/{contentAdaptationService.getPlatformLimits(platform)?.maxCharacters || 0}
                            </div>
                          </div>
                        )}
                      </div>
                    ...
                    {adaptedContents[platform] && (
                      <div className="bg-gray-50 rounded-lg p-4">
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <div className="flex items-center space-x-4 text-sm text-gray-600">
                            ...
                            <span className={`px-2 py-1 rounded-full text-xs ${
                              adaptedContents[platform].characterCount <= (contentAdaptationService.getPlatformLimits(platform)?.maxCharacters || 0)
                        ...
                        {adaptedContents[platform].adaptations.length > 0 && (
                          <div className="mt-3">
                            <h6 className="text-sm font-medium text-gray-700 mb-2">Adaptations Applied:</h6>
                            <div className="flex flex-wrap gap-2">
                              {adaptedContents[platform].adaptations.map((adaptation: string, index: number) => (
                                <span
                                  key={index}
                        ...
                        {adaptedContents[platform].warnings.length > 0 && (
                          <div className="mt-3">
                            ...
                            <div className="space-y-1">
                              {adaptedContents[platform].warnings.map((warning: string, index: number) => (
                                <div
                                  key={index}
                      ...
                      <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                        ...
                        {validations[platform].suggestions.length > 0 && (
                          <div className="mt-3">
                            ...
                            <ul className="space-y-1">
                              {validations[platform].suggestions.map((suggestion: string, index: number) => (
