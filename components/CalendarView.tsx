...
import { Post, Campaign, ContentSeries, TimeSlot, ConflictAnalysis, AudienceProfile } from '../types';
...
const CalendarView: React.FC<CalendarViewProps> = ({ 
    ...
    for (let i = 1; i <= daysInMonth; i++) {
        ...
        const isOptimalTime = optimalTimes.some(timeSlot => 
            timeSlot.dayOfWeek === date.getDay() && showOptimalTimes
        );
        
        // Check for conflicts on this date
        const hasConflicts = conflictAnalysis?.conflicts.some(conflict => 
    ...
    const getSeriesColor = (seriesId: string) => {
        ...
        const index = seriesId.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0) % colors.length;
        return colors[index];
    };
                                ...
                                <>
                                    ...
                                    <div className="flex justify-between items-start mb-1">
                                        ...
                                        <div className="flex space-x-1">
                                            ...
                                            {dayInfo.hasConflicts && (
                                                <div className="w-2 h-2 bg-red-400 rounded-full" title="Scheduling conflicts"></div>
                                            )}
                                        </div>
                                    </div>
